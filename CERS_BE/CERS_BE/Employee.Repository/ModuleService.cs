...
namespace Employee.Repository
{
    public class ModuleService : IModuleRepository
    {
        ...
        public async Task<Response> Select(int ModuleID)
        {
            try
            {
                using (var connection = new SqlConnection(_connectionString))
                {
                    ...
                    var results = await connection.QueryAsync<ModuleClass>("[dbo].[SelectModules]", para, commandType: CommandType.StoredProcedure);
